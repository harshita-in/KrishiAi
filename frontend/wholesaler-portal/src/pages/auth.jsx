    ...
    <div className="auth-shell">
      ...
      <div className="auth-card">
        ...
        <div className="auth-header">
          ...
          <h1>{title}</h1>
          <p>Wholesaler Portal access, built to match the farmer side exactly.</p>
        </div>
        <form className="auth-form" onSubmit={submit}>
          {mode === 'signup' && (
            <>
              <label>Name<input name="name" value={form.name} onChange={onChange} required /></label>
              <label>Create ID<input name="user_id" value={form.user_id} onChange={onChange} required /></label>
              <label>Email<input name="email" type="email" value={form.email} onChange={onChange} required /></label>
            </>
          )}
          {mode === 'login' && <label>ID<input name="user_id" value={form.user_id} onChange={onChange} required /></label>}
          ...
          <button className="auth-submit" type="submit" disabled={loading}>{loading ? 'Please wait...' : mode === 'signup' ? 'Create account' : 'Login'}</button>
